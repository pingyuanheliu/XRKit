...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if (self.yOffset != -9999.0) {
        ...
        if (offset < 5.0) {
            self.readyLoad = YES;
            NSLog(@"==+A+==:%f==%f==%f",scrollView.contentOffset.y,self.yOffset,offset);
        }else {
            self.readyLoad = NO;
            NSLog(@"==+B+==:%f==%f==%f",scrollView.contentOffset.y,self.yOffset,offset);
        }
    }else {
        self.readyLoad = YES;
    }
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    if (!self.isBegin) {
        if (!decelerate) {
            ...
            [self.listCV reloadItemsAtIndexPaths:array];
        }else {
            NSLog(@"==2==");
        }
    }else {
